    ...
    def back_propagation(self, data, expected_value):
        for layer_index in range(len(self.layers) - 1, -1, -1):
            ...
            for index, neuron in enumerate(self.layers[layer_index].neurons):
                ...
                if self.verbose:
                    print(
                        f"Layer index: {layer_index} | Neuron Index: {index} | Neuron.output: {neuron.output} | Expected value: {expected_value} | Neuron error: {neuron_error}")

                gradient = neuron.calculate_gradient(inputs, neuron_error)
                updated_weights = self.optimizer.update(neuron.weights,
                                                        gradient)  # Actualizar pesos usando el optimizador
